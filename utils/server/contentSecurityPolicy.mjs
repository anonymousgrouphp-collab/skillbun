export function buildContentSecurityPolicy({ nonce, production = process.env.NODE_ENV === 'production' } = {}) {
  ...
  const firebaseAuthOrigins = new Set([`https://${projectId}.firebaseapp.com`]);
  const authDomain = (process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN || '').trim();
  // Firebase expects a hostname, not a URL. Allow only the configured host;
  // never interpolate paths, credentials, wildcards, or CSP directives.
  if (/^(?=.{1,253}$)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)*[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/i.test(authDomain)) {
    firebaseAuthOrigins.add(`https://${authDomain.toLowerCase()}`);
  }
  let posthogOrigin = '';
  try {
  ...
  return [
    ...
    `frame-src 'self' blob: data: ${[...firebaseAuthOrigins].join(' ')} https://challenges.cloudflare.com https://accounts.google.com https://www.youtube.com https://youtube.com`,
