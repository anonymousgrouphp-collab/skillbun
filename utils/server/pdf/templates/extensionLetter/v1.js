 ...
 */
export async function generateExtensionLetterV1(employee, options = {}) {
  ...
  const rawRefId = options.referenceId || employee.reference_id || meta.reference_id || generateWorkforceId(WORKFORCE_PREFIXES.EXTENSION);
  const referenceId = formatWorkforceDisplayId(rawRefId);
  const originalRefId = formatWorkforceDisplayId(options.originalReferenceId || employee.offer_reference_id || employee.original_offer_id || meta.original_reference_id || 'SKB/2026/HR-OFF/ORIGINAL');
  ...
  const newContractEndDate = formatDate(options.newContractEndDate || meta.extended_contract_end_date || employee.contract_end_date);
  ...
  const metadataSnapshot = {
    ...
    extended_contract_end_date: options.newContractEndDate ? String(options.newContractEndDate) : (meta.extended_contract_end_date ? String(meta.extended_contract_end_date) : (employee.contract_end_date ? (employee.contract_end_date.toDate ? employee.contract_end_date.toDate().toISOString() : String(employee.contract_end_date)) : '')),
