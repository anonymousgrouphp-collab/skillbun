 ...
 */
export async function generateOfferLetterV1(employee, options = {}) {
  ...
  const rawRefId = options.referenceId || employee.offer_reference_id || employee.reference_id || meta.reference_id || generateWorkforceId(WORKFORCE_PREFIXES.OFFER);
  ...
  const parentName = meta.parent_name || employee.parent_name || 'Parent / Guardian';
  const currentAddress = meta.current_address || employee.current_address || 'Address on record';
  const permanentAddress = meta.permanent_address || employee.permanent_address || currentAddress;
  const courseDegree = meta.course_degree || employee.course_degree || 'Undergraduate Degree';
  const collegeName = meta.college_name || employee.college_name || 'University / Institution';
  const department = meta.department || employee.department || 'Operations & Management';
  const designation = meta.designation || employee.designation || 'Engineering Intern';
  const joiningDate = formatDate(meta.joining_date || employee.joining_date);
  const contractEndDate = formatDate(meta.contract_end_date || employee.contract_end_date);
  const stipendAmount = typeof (meta.stipend_amount ?? employee.stipend_amount) === 'number'
    ? (meta.stipend_amount ?? employee.stipend_amount)
    : 0;
  const stipendCurrency = meta.stipend_currency || employee.stipend_currency || 'INR';
  ...
  const metadataSnapshot = {
    ...
    course_degree: courseDegree || 'Undergraduate Degree',
    college_name: collegeName || 'University / Institution',
    personal_email: employee.personal_email || meta.personal_email || '',
    department: department || 'Engineering',
    designation: designation || 'Intern',
    joining_date: employee.joining_date ? (employee.joining_date.toDate ? employee.joining_date.toDate().toISOString() : String(employee.joining_date)) : (meta.joining_date || ''),
    contract_end_date: employee.contract_end_date ? (employee.contract_end_date.toDate ? employee.contract_end_date.toDate().toISOString() : String(employee.contract_end_date)) : (meta.contract_end_date || ''),
    stipend_amount: stipendAmount,
    stipend_currency: stipendCurrency || 'INR',
