...
- On every push / commit of code changes, increment the project version in `package.json` by `0.0.1`. Note: The inline `Project Changelog` section in `README.md` has been removed and must not be re-added.
...
1. **Logo Image Asset**: Always use `<Image src="/logo.png" alt="SkillBun Logo" ... />` (or `splash-logo.png` for splash/loading experience). Raw system emojis (e.g. 🐰, 🐾) must NEVER be used as a replacement for the official logo image asset.
2. **Clean Wordmark Typography**: Always render clean standard text `SKILLBUN` (or `SkillBun`) inside a `<span>` or heading styled with `font-family: var(--font-fredoka), cursive, sans-serif;` and brand green color (`var(--green)` or `#11864f`), bold weight (`font-weight: 700`), and clean letter-spacing (`letter-spacing: 0.04em` or `1px`). **NEVER use fake Unicode symbols** (such as Yi Syllables `ꌗꀘꀤ꒒꒒ꌃꀎꈤ` or symbol pseudofonts) as they render distorted, upside-down, unreadable, and break screen reader accessibility.
3. **Lockup Alignment**: The logo image and wordmark text must be aligned inside a flex container (`display: flex` or `inline-flex`, `align-items: center`, `gap: 0.5rem` to `0.65rem`), with proportional font sizes and image dimensions.
...
## SkillBun Career Discovery Quiz V2 System Contract

The Career Discovery Quiz V2 is an adaptive 10-question diagnostic engine designed for beginner/student career discovery ("Kya tujhe ye EXCITE karta hai?"). Maintain the following specifications:

1. **Question Motive & Format**:
   - **No Technical Right/Wrong Answers**: Every question presents 4 valid choices mapping to different career domains/pillars.
   - **Relatable Student Scenarios**: All questions use 1st-year friendly college hackathon, startup role, and team project contexts.
   - **Dynamic Masking**: The `{name}` placeholder is dynamically interpolated in question text, mentor insights, and options using the student's onboarding profile.

2. **2,531 Question Bank Structure**:
   - **Phase 1 (Core DNA)**: 8 broad discovery questions across 6 pillars (`systems`, `data_ai`, `design_product`, `cloud_infra`, `security`, `operations`).
   - **Phase 2 (Pillar Scenarios)**: 2,515 cert-derived relatable scenarios covering all 100 roadmaps (25 questions per roadmap).
   - **Phase 3 (AI Niche Deep-Dive)**: Q8 dynamically generated by Gemini API (or seamless local fallback).
   - **Phase 4 (Confirmation)**: Q9-Q10 final confirmation questions leading to Top 3 career recommendations.

3. **Onboarding Profile Pre-Sorting**:
   - `quizRuntime.js` maps onboarding `interest` and `degree` to initial `pillarScores` and skips Phase 1 for students with pre-declared interests.

4. **SBV1 Encryption for Quiz Data**:
   - Encrypted Quiz Pool: `content/quiz/questions.sbv`.
   - Encryption Script: `scripts/encrypt-quiz.js` (uses master `DOCS_ENCRYPTION_KEY` + XOR pepper + HKDF key derivation + AES-256-GCM).
   - API Endpoint: `GET /api/quiz/questions`.
   - Authentication: Requires `Authorization: Bearer <firebase_id_token>` header. Unauthenticated requests return `401 Unauthorized`.
   - Rules: Never serve unencrypted quiz questions publicly. Never expose decryption keys in repository files or client bundles.
